...
# read in datasets
rbt_data <- read_csv("rbt_vic.csv")
crash_data <- read_csv("Crashes_Last_Five_Years.csv")

# remove unnecessary columns and rename columns
rbt_data <- rbt_data %>% 
  select(-State) %>%
  rename(conducted_rbt = `RBT conducted`,
         positive_rbt = `Positive RBT`) %>%
  select(Year, positive_rbt, conducted_rbt)

# remove unnecessary columns, drop missing rows and rename columns
...
# convert date strings into date objects during lubridate
crash_data$ACCIDENT_DATE <- dmy(crash_data$ACCIDENT_DATE)
crash_data$Year <- year(crash_data$ACCIDENT_DATE)
crash_data$Month <- month(crash_data$ACCIDENT_DATE)
crash_data$Day <- day(crash_data$ACCIDENT_DATE)

# fix data anomaly: murrindindi wrong city type
crash_data[crash_data$LGA_NAME == "MURRINDINDI", ]$STAT_DIV_NAME <- "Country"

# dataset for proportional symbol map
...
# join crash and rbt data
crash_rbt_data <- inner_join(crash_data, rbt_data, by = "Year")

# dataset for line chart
# get alcohol related crashes by year
alcoholcrash_years <- crash_rbt_data %>%
  group_by(Year) %>%
  filter(ALCOHOL_RELATED == "Yes") %>%
...
# join with rbt data
alcoholcrash_rbt_years <- inner_join(alcoholcrash_years, rbt_data, by = "Year")

# remove 2013 and 2019 due to incomplete data for the year
alcoholcrash_rbt_years <- alcoholcrash_rbt_years[alcoholcrash_rbt_years$Year != 2019, ]
alcoholcrash_rbt_years <- alcoholcrash_rbt_years[alcoholcrash_rbt_years$Year != 2013, ]

# calculate percentage change 
alcoholcrash_rbt_years <- alcoholcrash_rbt_years %>%
  mutate(accident_perc_change = (accident_count - lag(accident_count))/accident_count,
         positive_rbt_perc_change = (positive_rbt - lag(positive_rbt))/positive_rbt,
         conducted_rbt_perc_change = (conducted_rbt - lag(conducted_rbt))/conducted_rbt
  )

# first year will have no previous year to compare against and have a null value
# change null to 0
alcoholcrash_rbt_years[is.na(alcoholcrash_rbt_years)] <- 0

# format dates into mm-yy
crash_data <- crash_data %>%
  mutate(m_y_date = format(as.Date(ACCIDENT_DATE, "%Y-%m-%d"), "%m/%y"))

# abbreviate months
crash_data$Month <- substr(months(crash_data$ACCIDENT_DATE), 1,3)
# change to categorical and reorder
crash_data$Month <- factor(crash_data$Month, levels= c('Jan', 'Feb', 'Mar', 'Apr', 'May',
                                                       'Jun', 'Jul', 'Aug', 'Sep', 'Oct',
                                                       'Nov', 'Dec'))
...
ui <- fluidPage(
  ...
  Student ID: 25200623 <br>
  Last Modified: 12/11/2020 <br> <br>"),
  ...
  <a href=https://discover.data.vic.gov.au/dataset/crashes-last-five-years>
...
server <- function(input, output, session){
  ...
  showModal(modalDialog(
    ...
    with this highly interactive and customisable visualisation!")))
  ...
  observeEvent(c(input$select_city_type, input$select_year), {
